...
This R markdown html is hosted as a github page, you can view the repository here: https://github.com/mikerspencer/training/tree/gh-pages which also includes the raw markdown file (Rmd) this webpage is created from.
...
Backing up your work is crucial. If you lose everything pity from your colleagues will not help get it back. Hopefully most of you will be working on the school server (adder/m drive), this is backed up. Another option could be that you work on your desktop and backup to the work server. If you do this copying folders across is so 1995. Use backup software like [rsync](http://en.wikipedia.org/wiki/Rsync) which scans for changes in files and only copies over what is needed. It's very fast. Here's some examples that you would run in a bash terminal:

```{r backup, engine="bash", eval=F}
# Back up to an external drive
rsync -azv /home/username/ /media/username/drivename

# Add other options like:
# Delete files that no longer exist
# Exclude certain directories
# Back up across the network (to adder)
rsync --delete --exclude='/.*/' --exclude='/dir/temp/' -azv -e ssh /home/s1066252 s1066252@adder.geos.ed.ac.uk:/home/s1066252/
```

The above example doesn't just work within the school, you can also use it at home or wherever. Remember to change the student number to your own! For more settings and options see: [rsync manual](http://linux.about.com/library/cmd/blcmdl1_rsync.htm).
...
The key is to get familiar with the language and concepts of GIS, don't get bogged down in learning which buttons to press.

### GDAL

[GDAL](http://www.gdal.org/) is a suit of tools for translating spatial data. Most GIS software uses it (e.g. Arc, QGIS and GRASS do, MapInfo does not). It is written in Python so you can call it natively from there, there are also packages for R to use it.

When using from a GIS, it's used to read, write and reproject data primarily. In QGIS and GRASS it's fairly obviously implemented as these tools are labelled with GDAL! Using via the command line (bash, Python, R, etc.) you're better able to automate GDAL to process a large number of files.

GDAL can be used with [vector data](http://www.gdal.org/ogr_utilities.html) and [raster data](http://www.gdal.org/gdal_utilities.html). In the following examples I'll show it being used with raster data, as I find these more useful for working with images. Here are the basics for working with raster files:

* [gdal_translate](http://www.gdal.org/gdal_translate.html), for converting raster file formats
* [gdalwarp](http://www.gdal.org/gdalwarp.html), for reprojecting rasters
* [gdal_merge](http://www.gdal.org/gdal_merge.html), for mosaicing raster, see also [gdalbuildvrt](http://www.gdal.org/gdalbuildvrt.html).

The script example below shows these tied together in a script using R, but it would not take long to convert these to another language. This example:
...
I developed this to convert MODIS snow cover hdf files to tifs and merge the pairs of tiles together, finally converting them to OSGB grid. It loops through 14 years of daily observations by two satellites, processing ~ 
18'000 raster images in total.

```{r gdal example, eval=F}
# ---------------------------------------
# Raster merge script
# ---------------------------------------
...
Have a look at using SQLite, there are packages for Python and R and with an extension you can link it to a GIS. You can even get a [plugin for Firefox](https://addons.mozilla.org/en-Us/firefox/addon/sqlite-manager/) to query your data. Have a look at the example workflow at the bottom of this document to see a database in action.
...
For tips on using LaTeX with version control see this answer on [stackoverflow](http://stackoverflow.com/a/6190412/2773500).
